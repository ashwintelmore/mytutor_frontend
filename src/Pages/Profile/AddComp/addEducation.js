...
function Education({ show, setShow, }) {
  ...
  return (
    <div className="flex w-full  items-center justify-center xs:flex-col fixed overflow-scroll h-screen top-0 left-0 z-[12]">
      <div className=" bg-white dark:bg-color-11 transition-all duration-500 ease-in-out dark:border dark:text-white z-[9] absolute my-[10%] top-0 px-6  w-4/6 h-auto py-10 rounded-3xl flex flex-col  items-center justify-center  shadow-md shadow-color-8 xs:flex-col xs:w-11/12">
        <div className="flex w-full">
          <h2 className="text-color-4 text-3xl sm:text-xl  font-medium px-4 py-3  ">Education</h2></div>
        ...
        <div className="flex p-1   w-full justify-between text-sm xs:text-xs xs:gap-0 xs:p-1 xs:flex-col xs:w-full sm:flex-col sm:w-full">
          <div className="flex flex-col relative  w-[45%] p-2  xs:w-full sm:w-full">
            ...
            <input className="rounded-xl dark:bg-color-11 dark:border w-full shadow-sm shadow-black p-2" type="text"
              name="title"
          ...
          <div className="flex flex-col   w-[45%] p-2 xs:w-full sm:w-full">
            ...
            <input className="rounded-xl w-full dark:bg-color-11 dark:border shadow-sm shadow-black p-2" type="text"
              name="orginization"
              onChange={(e) => setUserData({ ...userData, [e.target.name]: e.target.value })}
        ...
        <div className="flex p-1   w-full justify-between text-sm xs:text-xs xs:gap-0 xs:p-1 xs:flex-col xs:w-full sm:flex-col sm:w-full">
          <div className="flex flex-col   w-[45%]  p-2  xs:w-full sm:w-full  ">
            ...
            <input className="  rounded-xl w-full dark:bg-color-11 dark:text-white dark:border  p-2 shadow-sm shadow-black" type="date"
              name="from"
              onChange={(e) => setUserData({ ...userData, [e.target.name]: e.target.value })}
              value={userData.from}
            />
          ...
          <div className="flex flex-col  xs:w-full  w-[45%] p-2 justify-around sm:w-full ">
            ...
            <input className="  rounded-xl w-full p-2 dark:bg-color-11 dark:text-white dark:border  shadow-sm shadow-black" type="date"
              name="to"
              onChange={(e) => setUserData({ ...userData, [e.target.name]: e.target.value })}
              value={userData.date}
            />
        ...
        <div className="flex p-1    w-full justify-between text-sm xs:text-xs xs:gap-0 xs:p-1 xs:flex-col xs:w-full">
          ...
          <div className="flex  flex-col  w-[45%]  p-2 xs:w-full sm:w-full">
            ...
            <input className="  rounded-xl w-full p-2 dark:bg-color-11 dark:border shadow-sm shadow-black" type="text"
              name="location"
              onChange={(e) => setUserData({ ...userData, [e.target.name]: e.target.value })}
        ...
        <div className="p-1">
          <h2>Description :</h2>
          <textarea rows={5} cols={100} className="rounded-2xl px-2 py-1 xs:h-24 w-4/5 xs:w-11/12 shadow-sm dark:bg-color-11 dark:border shadow-black sm:w-full"
            name="descrp"
            onChange={(e) => setUserData({ ...userData, [e.target.name]: e.target.value })}
